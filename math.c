...
void InfPerspectiveDxDy(float fovy, float aspect, int width, int height, float zNear, float pixdx, float pixdy, float *out)
{
	float y=tanf((fovy/2.0f)*3.14159f/180.0f)*zNear, x=aspect*y;
	float yy=-y, xx=-x, m[16], nudge=1.0f-(1.0f/(1<<20)), dx, dy;

	dx=pixdx*(x-xx)/width;
	dy=pixdy*(y-yy)/height;
...
void PerspectiveDxDy(float fovy, float aspect, int width, int height, float zNear, float zFar, float Dx, float Dy, float *out)
{

...
int indexBinaryTraverse(void *value, void *data, int *indices, int stride, int count, int *result, int (*compare)(const void *, const void *))
{
	...
	*result=-1;
	...
	while(low!=high)
	{
		*result=compare(value, (void *)((unsigned long)data+indices[mid]*stride));
...
void CalculateTangent(Mesh3DS_t *Mesh)
{
	...
	for(i=0;i<Mesh->NumFace;i++)
	{
		...
		for(j=0;j<3;j++)
		{
			...
			Mesh->Normal[3*Mesh->Face[3*i+j]+0]*=inv;
			Mesh->Normal[3*Mesh->Face[3*i+j]+1]*=inv;
			Mesh->Normal[3*Mesh->Face[3*i+j]+2]*=inv;
		}
	}
